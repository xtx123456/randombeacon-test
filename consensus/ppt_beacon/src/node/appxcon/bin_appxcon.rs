...
impl Context{
    ...
    pub async fn next_round_begin(&mut self,round:Round){
        ...
        if round_begin >= 1 && self.round_state.contains_key(&(round_begin-1)) && self.round_state.get(&(round_begin-1)).unwrap().round_state.contains_key(&(round-1)){
            ...
            log::error!("[PPT][OLD-BAA] Terminated beacon for round {} with committee {:?} and appxcon_vals: {:?}, term_secrets {:?}, comm_vector {:?}", round_begin-1,rbc_state.committee,rbc_state.appx_con_term_vals,rbc_state.terminated_secrets,rbc_state.comm_vectors.keys());
        ...
        if (round+1) % self.frequency == 0{
            ...
            log::error!("[PPT][OLD-BAA] Started round {} with Binary AA",round+1);
        }
    }
}