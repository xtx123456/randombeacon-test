...
impl Context {
    pub async fn process_gatherecho(self: &mut Context,wss_indices:Vec<Replica>, echo_sender:Replica,round: u32){
        ...
        log::info!("[PPT][OLD-GATHER] Received gather echo message {:?} from node {} for round {}",wss_indices.clone(),echo_sender,round);
    ...
    pub async fn process_gatherecho2(self: &mut Context,wss_indices:Vec<Replica>, echo_sender:Replica,round: u32){
        ...
        let rbc_state = self.round_state.get_mut(&round).unwrap();
        log::info!("[PPT][OLD-GATHER] Received gather echo2 message {:?} from node {} for round {}",wss_indices.clone(),echo_sender,round);
    ...
    pub async fn witness_check(self: &mut Context,round:Round){
        ...
        if !rbc_state.send_w2{
            ...
            if i >= self.num_nodes-self.num_faults{
                log::error!("[PPT][ACS-OBS] round {} witness1-complete local completed dealers = {:?}", round, rbc_state.terminated_secrets.clone());
            ...
            if i >= self.num_nodes-self.num_faults && !rbc_state.started_baa{
                ...
                log::error!("[PPT][ACS-OBS] round {} witness2-complete local completed dealers = {:?}", round, rbc_state.terminated_secrets.clone());
                log::error!("[PPT][ACS-SEND] node {} broadcasting ACSInit for round {} with dealers {:?}", self.myid, round, acs_dealers);
                msgs_to_be_sent.push(CoinMsg::ACSInit((self.myid, round, acs_dealers.clone())));
                log::error!("[PPT][OLD-GATHER] Accepted n-f witness2 for node {} with set {:?}",self.myid,rbc_state.terminated_secrets.clone());
                rbc_state.started_baa = true;
