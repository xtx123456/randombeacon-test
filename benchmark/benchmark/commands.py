...
class CommandMaker:
    ...
    @staticmethod
    def run_primary_ppt(key,delay,ep,delta,val,tri,batch,freq, debug=False):
        assert isinstance(key, str)
        assert isinstance(debug, bool)
        return (f'./node --config {key} --ip ip_file '
                f'--sleep {delay} --vsstype ppt --epsilon {ep} --delta {delta} --val {val} --tri {tri} --syncer syncer --batch {batch} --frequency {freq}')
