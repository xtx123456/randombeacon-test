...
use ff::Field;
use crate::crypto::{PolyCommit, VecCommit, Polynomial};
use crate::crypto::anyhow::Result;
...
impl VecCommit for BlstrsKZGVec {
    ...
    fn commit<T: AsRef<[u8]>>(&self, vec: &Vec<T>) -> Result<Self::Commitment> {
        ...
        let poly = Polynomial::new(crate::crypto::interpolate(&xs, ys, |s| s.invert().unwrap(), Scalar::zero()));
